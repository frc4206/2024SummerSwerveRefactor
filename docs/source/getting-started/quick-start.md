...
_battleaid_ is not published on Maven as a public package (yet), and is only available as a _GitHub package_.  This requires more setup by the user, but the result functions exactly as any other remote resource.
...
```
5. Add _battleaid_ to your list of dependencies:
```
dependencies {
    ...
...
```{tip}
If the build is successful but you still see squiggly red errors in your code, you may need to refresh the Java Lanaguage Server in order for _battleaid_ to be visible:
- `Cntrl` + `Shft` + `P`
...
If all of the above steps are performed correctly, _battleaid_ will be listed as an external dependency in the `Java Projects` primary sidebar view container.